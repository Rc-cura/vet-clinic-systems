      ...
      <ScrollView contentContainerStyle={{ paddingHorizontal: 25, paddingBottom: 40 }}>
        ...
        <View style={{ alignItems: 'center' }}>
          ...
          {services.map((item) => {
            ...
            return (
              <TouchableOpacity key={item.id}  activeOpacity={0.8}style={[ MyStyleSheet.serviceCard, isSelected && { borderColor: '#5C93E8', borderWidth: 2 }  ]} 
                onPress={() => toggleService(item.id)}>
                <Image  source={item.icon}  style={{ width: 50, height: 50, marginBottom: 10 }}  resizeMode="contain"   />
        ...
        <TouchableOpacity style={[ MyStyleSheet.primaryBlueBtn, { marginTop: 1 }, selectedServices.length === 0 && { backgroundColor: '#F0F0F0' }
          ]} 
          disabled={selectedServices.length === 0}
          onPress={() => {
            // FIXED: eto naayos ko na guys
            ...
            opx.navigate('datetime', { petName, petImage,  petDetails, petWeight, service: allSelectedNames })
          }}>
          <Text style={[MyStyleSheet.primaryBlueBtnText, selectedServices.length === 0 && { color: '#CCC' }]}>Continue
          </Text>
        </TouchableOpacity>
